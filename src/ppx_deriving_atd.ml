...
let collect_atd_strings_and_export strs =
  ...
  let atd_export_init =
    let gen_string = Printf.sprintf "(*** GENERATED BY PPX_DERIVING_ATD *** DO NOT EDIT ***)" in
    [
      [%stri let __ATD_EXPORT_LIST = ref [[%e Ast_builder.Default.estring ~loc gen_string]] ];
      [%stri let __add_to_export_list s = __ATD_EXPORT_LIST:= s :: !__ATD_EXPORT_LIST];
      [%stri let atd_file_string () = String.concat "\n" (List.rev !__ATD_EXPORT_LIST)];
      [%stri let export_atd_file filename =
